...
def seqence_context_synergy_quantification(args, final_motif_list, motif_pos_dict, motif_count_dict, encoded_seq, model, ori_pred_result):
    ...
    for i, motif in enumerate(final_motif_list):
        ...
        tmp_result = pred_result[tmp_idx[0]:tmp_idx[1]]
        scs_score = seqence_context_synergy(ori_pred_result, tmp_result, motif_pos_dict[motif])
        if args.top_r == 'all':
...
def functionality_quantification(args, Seqs):
    ...
    logger.info("Predicting original sequences...")
    ori_pred_result = tf_model_predictor(model, encode_seqs)
    if args.category != 'all':
        ori_pred_result = ori_pred_result[:, int(args.category)]
    ...
    for i, chunk in enumerate(tqdm(chunks)):
        chunk_scs_results = seqence_context_synergy_quantification(args, chunk, motif_pos_dict, motif_count_dict, encode_seqs, model, ori_pred_result )
